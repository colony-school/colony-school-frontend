...
const NavRail = ({
  ...
  return (
    <nav className="flex flex-col items-center gap-1 w-fit h-screen p-1 bg-light-surface1">
      <Link href="/">
        <a className="relative w-10 m-4 aspect-square">
          ...
          <Link href={item.url}>
            <a className="flex flex-col justify-center items-center gap-1 p-2 h-20 group">
              <div
                ...
                group-hover:bg-light-tertiary-container group-hover:transition-none
                ${
                  path == item.url
                    ? "text-light-tertiary bg-light-secondary-container dark:text-light-tertiary dark:bg-dark-secondary-container"
                    : "bg-transparent"
                }`}
              >
                {item.icon}
